  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-safe-left pr-safe-right">
        <h2 className="font-display text-3xl md:text-4xl font-bold text-text-primary mb-8 text-center">
          Shop by Category
        </h2>
        ...
        >
          {loading ? (
            [0, 1, 2, 3, 4, 5, 6, 7].map((i) => (
              <div
                key={i}
                className="bg-gray-200 p-6 rounded-lg h-32 animate-pulse"
              />
            ))
          ) : categories.length > 0 ? (
            categories.map((category) => {
              ...
              return (
                <motion.button
                  ...
                  className="group relative bg-white p-6 rounded-lg border border-border-light hover:border-primary hover:shadow-lg transition-all duration-300 flex flex-col items-center gap-3 cursor-pointer"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  <div className="text-4xl">{emoji}</div>
                  <p className="text-sm font-semibold text-text-primary text-center group-hover:text-primary transition-colors">
                    {decodedName}
                  </p>
